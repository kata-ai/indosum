...
flags.DEFINE_string('which',        'test',       'which dataset to summarize (this should be a directory inside data_dir)')
...
def main(_):
    ...
    test_reader = DataReader(word_tensors[FLAGS.which], label_tensors[FLAGS.which],
                              FLAGS.batch_size)
    ...
    with tf.Graph().as_default(), tf.Session() as session:
        ...
        for x, y in test_reader.iter():
            ...
            # shape: (batch_size, max_doc_length, num_labels)
            logits = np.stack(logits, axis=1)
            # shape: (batch_size, max_doc_length)
            pred = logits.argmax(axis=-1)
            preds.append(pred)

        # shape: (batch_size, test_reader.length, max_doc_length)
        preds = np.stack(preds, axis=1)
        # shape: (batch_size * test_reader.length, max_doc_length)
        preds = preds.reshape(-1, preds.shape[-1])
        print('writing summary')
        save_as = os.path.join(FLAGS.train_dir, 'summary.txt')
        with codecs.open(save_as, 'w', encoding='utf-8') as f:
            for filename, pred in zip(os.listdir(os.path.join(FLAGS.data_dir, FLAGS.which)), preds):
                id_, _ = os.path.splitext(filename)
                print(id_, file=f, end='\t\t\t')
                print(' '.join([str(p) for p in pred]), file=f)
        print('summary written to', save_as)
